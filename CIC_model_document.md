...
### 3.5 New Model3 — Smooth-Trend Adaptive Forecaster

**Change from D1**: Model3 introduces two enhancements over D1:

1. **Trailing-window OLS (Step 1)**: Instead of estimating seasonal betas over the full history, Model3 fits OLS only on the most recent 60 calendar months (~5 years). This allows seasonal coefficients to adapt to recent patterns (e.g., post-COVID spending shifts, digital-payment adoption). The 60-month window is tunable.

2. **Smooth-trend UC (Step 2)**: Instead of a local-level UC, Model3 uses `level='smooth trend'` (Harvey 1989), which fixes the level variance to zero and allows the slope to drift slowly. This directly targets the end-of-month level KPI by producing slower-moving level forecasts with reduced volatility.

**Specification**:

- **Step 1**: OLS on the trailing 60-month window using the same 55-dummy matrix as Old_2022:
  $$\hat{\boldsymbol{\beta}}_{t} = \text{OLS on } \{(X_s, \Delta\text{CIC}_s) : s \in [t-60\text{m}, t]\}$$
- **Step 2**: UC smooth-trend on full-history OLS residuals:
  $$\text{resid}_t = \ell_t + u_t, \quad \ell_t = \ell_{t-1} + b_{t-1}, \quad b_t = b_{t-1} + \zeta_t$$
  with autoregressive(1) component. Level variance fixed to 0; slope variance estimated.

**Key distinction from D1**:
- D1 uses a local-level state (allows mean to jump); good for capturing COVID shock.
- Model3 uses a smooth-trend state (slope drifts, level is continuous); better for slow structural shifts in CIC trend.

**Expected use case**: Post-COVID normalisation period where the CIC trend is shifting gradually (declining from digital-payment adoption). Model3's trailing-window OLS adapts seasonal betas to recent data, while the smooth-trend state tracks the slow drift.

---

### 3.6 GARCH(1,1) — Variance Model (Direction 2)
...
All three models (Old_2022, D1, Model3) decompose each forecast into two additive components:

| Component | Notation | Source | Meaning |
|-----------|----------|--------|---------|
| Comp1_Calendar | $\hat{y}_t^{(1)}$ | OLS prediction on future dummy matrix | Deterministic seasonal/calendar effect |
| Comp2_Drift | $\hat{y}_t^{(2)}$ | ARIMA or UC prediction on residuals | Stochastic drift / adaptive component |

The monthly EOM forecast sum decomposes as:
$$\text{Forecast EOM} = L_{\text{last}} + \sum_t (\hat{y}_t^{(1)} + \hat{y}_t^{(2)})$$

This decomposition is tracked in the `Component_Decomp` and `Component_Summary` Excel sheets, providing diagnostic insight into how much of the forecast accuracy comes from the calendar structure vs. the drift correction.

**Expected patterns**:
- Old_2022: Comp2 (ARIMA drift) is small and mean-reverting; most forecast variation from Comp1.
- D1: Comp2 (Kalman state) adapts to the current CIC regime; larger Comp2 RMSE during COVID.
- Model3: Comp1 adapts via trailing-window OLS; Comp2 (smooth-trend) drifts slowly.

### 4.4 Pre-COVID Validation (cfg_precovid)

A third evaluation configuration tests model performance in a **pure out-of-sample pre-COVID period**:

| Config | Train | OOS | Purpose |
|--------|-------|-----|---------|
| cfg_benchmark | 1997–2021 | Dec 2021–May 2022 | BOT paper comparison |
| cfg_main | 1997–2019 | Jan 2020–present | COVID + post-COVID OOS |
| **cfg_precovid** | **1997–2017** | **2018–2019** | **Pure OOS, normal regime** |

**Rationale**: The 2018–2019 period is a clean, pre-COVID normal-regime hold-out with no structural breaks. A model that performs poorly here (relative to in-sample fit) likely overfits the training data or has high parameter uncertainty. This validation is particularly important for Model3, which uses trailing-window OLS — if the trailing window is too short, the model may adapt aggressively and overfit recent data.

The EOM level backtest (`month_end_eom_backtest`) is run for 2018–2019 separately as `eom_results_precovid`, merged into fig11 and fig12 bar charts.

### 4.6 GARCH Supplement

If ARCH-LM test confirms heteroscedasticity (expected), fit GARCH(1,1) on Old_2022 residuals to generate prediction intervals for the monthly monitor report. Point forecasts remain from the ARMA mean equation.
...
| `fig10_trend_slope.png` | Model D1 smoothed adaptive drift ν_t over training period |
| `fig11_eom_level.png` | Actual vs forecast EOM CIC level — all 3 models (2020–2025 + pre-COVID bar chart) |
| `fig12_model_comparison.png` | 3-panel: OOS daily ΔCIC / rolling RMSE bars / EOM RMSE bars — all 3 models |
