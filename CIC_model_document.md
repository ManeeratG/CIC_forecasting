...
| `CIC_output.xlsx` | **Production workbook** (rebuilt 2026-08 around `--eom`'s own model rankings — `export_cic_output()`). 3 tabs, no RMSE anywhere: `Daily` (best daily model's forecast: current month to date + nowcast + pure forecast), `EOM` (best daily / best EOM / best blend, each column its own 1-month-ahead backtest history plus the 2 forecast rows), `Summary` — titled **"Monthly Change Forecast"** — top 3 models by holdout RMSE plus `Daily_Baseline` (`top_models_for_export()`), one column per forecast month, values are the monthly **change** each model implies, not the level. Diagnostic-only variants (e.g. `Monthly_BreakTrend_NoBreak`) never take a slot in any tab, even if they'd score well — see §5.7. |
| `cic_forecast_output.xlsx` | **Research workbook** — every RMSE/backtest number, every model, no narrowing: the daily pipeline's per-config eval rows, in-sample fits, rolling/horizon RMSE, GARCH params, **plus** (since 2026-08) `--eom`'s `EOM_Selection` / `EOM_Holdout` / `EOM_Detail` / `Daily_Guardrail` (`export_results()`, all models — the narrowing only applies to `CIC_output.xlsx`). Both writers use `_excel_writer()` (append/replace-by-sheet-name), so running the daily pipeline and `--eom` in either order accumulates sheets rather than one truncating the other's. |
...
| `fig3_seasonal_pattern.png` | EOM level by year, with two forecast horizons per family (incumbent / best daily / best EOM / best blend, named by actual model): a **nowcast** for the current month (real partial-month data + a forecast for the remaining days) and a **pure forecast** for the following month (no partial-month data at all), connected by a dotted line, each with its own zoom panel (models sorted max→min). **`--eom`.** |
...
python cic_forecast.py                            # daily pipeline → cic_forecast_output.xlsx + fig1-2
python cic_forecast.py --eom --gate0               # EOM harness: post-fix sanity check only
python cic_forecast.py --eom                       # EOM harness: full backtest → CIC_output.xlsx + cic_forecast_output.xlsx + fig3-6
