...
| `CIC_output.xlsx` | **The one Excel deliverable.** `Daily` / `Monthly EOM` / `Summary` (from the daily pipeline) plus `EOM_Selection` / `EOM_Holdout` / `EOM_Detail` / `Daily_Guardrail` (from `--eom`) — 7 sheets, one file. Both writers only replace the sheets they own (`_excel_writer()`), so running the daily pipeline and `--eom` in either order accumulates into this one workbook rather than overwriting each other. **The EOM_\* sheets are narrowed to the top 3 models by holdout RMSE plus `Daily_Baseline`** (the incumbent, added if it didn't already make the top 3) via `top_models_for_export()` — a production workbook should show the candidates worth weighing, not every research variant the harness happened to run that pass. Diagnostic-only variants (e.g. `Monthly_BreakTrend_NoBreak`, §5.7's ablation twin) never take one of the 3 slots even if they score well, since they exist to isolate a component of another model, not to be a candidate themselves. fig4/fig5a/b/c are unaffected by this — they always show every model that was actually run. |
